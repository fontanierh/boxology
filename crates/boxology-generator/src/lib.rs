...
pub fn generate(request: &GenerationRequest) -> Result<GeneratedTree, Diagnostics> {
    ...
    let mut files = OUTPUTS
        .iter()
        .zip([manifest.into_bytes(), rust.into_bytes(), schema])
...
"#;
    ...
    fn revision(source: &str) -> String {
        ...
        serde_json::from_slice::<Value>(tree.files()[2].bytes()).unwrap()["revision"]
            .as_str()
            .unwrap()
            .to_owned()
    }
...
"#;
        ...
        assert_eq!(generated.files()[2].bytes(), SCHEMA);
        ...
        let changed_provenance = schema::document("hello", contract.model(), &[7; 32], "9.9.9");
    ...
    fn public_revision_tracks_every_public_semantic_and_only_public_semantics() {
        ...
        let first_schema: Value =
            serde_json::from_slice(tree(&first, false).files()[2].bytes()).unwrap();
        assert_eq!(
        ...
        let metadata: Value =
            serde_json::from_slice(tree(metadata, false).files()[2].bytes()).unwrap();
    ...
    fn output_set_fails_closed_with_one_stable_code() {
        let cases = [
            ...
            (
                vec![OUTPUTS[0], OUTPUTS[1], OUTPUTS[2], "generated/extra"],
            ...
            (
                vec![OUTPUTS[0], OUTPUTS[1], OUTPUTS[2], OUTPUTS[2]],
    ...
    fn generator_inventory_duplicates_fail_closed() {
        ...
        let diagnostics = request
            .require_exact_outputs(&[OUTPUTS[0], OUTPUTS[1], OUTPUTS[2], OUTPUTS[2]])
            .unwrap_err();
        assert_eq!(diagnostics.as_slice()[0].code(), "BXG0039");
    }
}
