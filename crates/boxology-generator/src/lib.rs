...
pub fn generate(request: GenerationRequest) -> Result<GeneratedTree, Diagnostics> {
    ...
    let variants = error
        ...
        .collect::<String>();
    let error_eq = error.variants.iter().all(|variant| match &variant.payload {
        VariantPayload::Unit => true,
        VariantPayload::Value(value) => {
            !matches!(value.ty, CanonicalType::F32 | CanonicalType::F64)
        }
        VariantPayload::Named(_) => false,
    });
    let error_derives = if error_eq {
...
/// `cargo fmt --all` selects workspace members, including generated contract packages. Keep the
/// pinned prettyplease bytes authoritative by marking each complete top-level generated item as a
/// rustfmt skip. An inner crate attribute is unstable, and skipping individual nested fragments
/// would leave whitespace between them mutable, so every item receives its own stable outer
/// attribute before prettyplease prints the file.
fn protect_generated_items_from_rustfmt(file: &mut syn::File) {
    for item in &mut file.items {
        let attributes = match item {
            ...
            syn::Item::Verbatim(_) => {
                unreachable!("fixed generated source contains no unparsed top-level items")
            }
            _ => unreachable!("fixed generated source contains only known syn item variants"),
        };
        let marker: syn::ItemConst = syn::parse_str("#[rustfmt::skip] const _: () = ();")
            .expect("fixed rustfmt marker must parse");
        attributes.insert(0, marker.attrs.into_iter().next().unwrap());
    }
}
...
fn structured_type_source(declaration: &DataDeclaration, supports_eq: bool) -> String {
    ...
    match &declaration.shape {
        ...
        DataShape::Enum(variants) => {
            ...
            format!(
                r#"
                {attrs}#[derive(Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)] pub enum {name} {{ {definitions} Unknown {{ tag: ::std::string::String, payload: ::boxology_contract::OpaquePayload }} }}
                #[allow(deprecated)] impl ::core::default::Default for {name} {{ fn default() -> Self {{ Self::{first} }} }}
                impl ::boxology_contract::ContractType for {name} {{
                    fn encode_value(&self) -> ::core::result::Result<::boxology_contract::ContractValue, ::boxology_contract::EncodeError> {{
                        let (tag, payload) = match self {{
                            {encoders}
...
#[rustfmt::skip]
#[allow(deprecated)]
impl ::core::default::Default for GreetError {
    fn default() -> Self {
        Self::EmptyName
    }
}
...
"#;
    ...
    fn structured_type_template_is_public_ordered_and_byte_locked() {
        ...
        const SOURCE_SHA256: &str =
            "c8e019112a586bc9beef7ec9ba145617efc9e6e8c21ffc36686f88758599f0b7";
        let contract = scalar_model(SOURCE);
...
"#,
    ...
    fn cargo_proves_generated_boundary_and_stale_failure() {
        ...
        let abi = r#"
            ...
            fn assert_model_traits<T: Default + Eq + std::hash::Hash + Ord>() {}
