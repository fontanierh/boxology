...
pub fn generate(request: GenerationRequest) -> Result<GeneratedTree, Diagnostics> {
    ...
    let error_derives = if error_eq && default_is_derivable {
        "Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Default"
    } else if error_eq {
        "Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord"
    } else if default_is_derivable {
...
fn structured_type_source(declaration: &DataDeclaration, supports_eq: bool) -> String {
    ...
    match &declaration.shape {
        ...
        DataShape::Enum(variants) => {
            ...
            format!(
                r#"
                {attrs}#[derive(Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Default)] pub enum {name} {{ {definitions} Unknown {{ tag: ::std::string::String, payload: ::boxology_contract::OpaquePayload }} }}
                impl ::boxology_contract::ContractType for {name} {{
                    fn encode_value(&self) -> ::core::result::Result<::boxology_contract::ContractValue, ::boxology_contract::EncodeError> {{
                        let (tag, payload) = match self {{
                            {encoders}
...
"#;
    ...
    fn structured_type_template_is_public_ordered_and_byte_locked() {
        ...
        const SOURCE_SHA256: &str =
            "631779b58827f6becdc657f6ec2fbaa1903f6d143f83a639cce1c71c64e0cf2c";
        let contract = scalar_model(SOURCE);
