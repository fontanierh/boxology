...
use boxology_contract::{OpaqueNumber, OpaqueTree};

pub(crate) const DEFAULT_DEPTH_LIMIT: usize = 128;
...
pub(crate) fn parse(input: &[u8], limits: SyntaxLimits) -> Result<OpaqueTree, SyntaxError> {
    if input.len() > limits.0 {
        return Err(SyntaxError::PayloadTooLarge { limit: limits.0 });
    }
...
mod tests {
    ...
    fn limits(max_bytes: usize, max_depth: usize) -> SyntaxLimits {
        SyntaxLimits(max_bytes, max_depth)
    }
    ...
    fn parse_error(input: &[u8], limits: SyntaxLimits) -> SyntaxError {
