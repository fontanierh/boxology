...
const ROOT_INSTRUCTIONS: &str = include_str!("../../../AGENTS.md");
const BOXOLOGY_SKILL: &str = include_str!("../../../.agents/skills/boxology/SKILL.md");
const ROOT_START: &str = "===== BEGIN ROOT AGENTS.md =====\n";
const ROOT_END_SKILL_START: &str =
    "===== END ROOT AGENTS.md =====\n\n===== BEGIN .agents/skills/boxology/SKILL.md =====\n";
const SKILL_END: &str = "===== END .agents/skills/boxology/SKILL.md =====\n";
...
#[rustfmt::skip] #[derive(Deserialize)] #[serde(deny_unknown_fields)] struct Turn{session_id:String,turn_id:String,user_message:String,tools:Vec<Tool>,max_output_tokens:Option<u64>}
...
fn request(id:&str)->String{format!(r#"{{"schema":"{SCHEMA}","id":"{id}","method":"run_turn","params":{{"session_id":"s","turn_id":"t","user_message":"u","tools":[],"max_output_tokens":null}},"timeout_ms":2}}"#)}
...
#[test]fn static_context_is_exact_and_legacy_prompt_is_rejected(){let captured=Arc::new(Mutex::new(None));let seen=captured.clone();let h=AgentLoopFake::new().with_run_turn(move|_,request|{*seen.lock().unwrap()=Some(request);async{Ok(contract::RunTurnOutcome{result:Some(contract::RunTurnResult{answer:"ok".into(),tool_name:None,tool_call_id:None,tool_output_json:None,usage:contract::TurnUsage{input_tokens:0,output_tokens:0,total_tokens:0}}),failure:None})}}).handle();let legacy=request("legacy").replace(r#""user_message":"u","#,r#""user_message":"u","system_prompt":"caller controlled","#);let input=format!("{}\n{}\n",request("static"),legacy);let mut out=vec![];assert_eq!(serve(&h,input.as_bytes(),&mut out),0);let responses:Vec<Value>=String::from_utf8(out).unwrap().lines().map(|line|serde_json::from_str(line).unwrap()).collect();assert_eq!(responses[0]["result"]["answer"],"ok");assert_eq!(responses[1]["error"]["code"],"invalid_params");let request=captured.lock().unwrap().take().unwrap();let expected=[b"===== BEGIN ROOT AGENTS.md =====\n".as_slice(),include_bytes!("../../../AGENTS.md"),b"===== END ROOT AGENTS.md =====\n\n===== BEGIN .agents/skills/boxology/SKILL.md =====\n".as_slice(),include_bytes!("../../../.agents/skills/boxology/SKILL.md"),b"===== END .agents/skills/boxology/SKILL.md =====\n".as_slice()].concat();assert_eq!(request.system_prompt.as_bytes(),expected);}
