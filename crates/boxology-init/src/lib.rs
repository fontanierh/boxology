...
//! crate, and embedded S2-generated contract artifacts), and a Cargo workspace listing those
//! members. It performs no filesystem, runtime environment, network, clock, or process access; the
//! writer and later project packages are separate work.
...
const WORKSPACE_MEMBERS: &[&str] = &["ping/generated/contract", "ping/implementation"];
...
///
/// This slice uses literal-only output paths for the root platform files and the ping box package
/// files. Embedded generator paths are confined before the `ping/` prefix is applied. A writer must
/// pass every path through [`confined_destination`] before joining it to its destination.
#[derive(Debug, Eq, PartialEq)]
...
fn embed_ping() -> Result<Vec<GeneratedFile>, Diagnostics> {
    ...
    files.extend(embed(PING_MANIFEST, PING_IMPLEMENTATION)?);
    Ok(files)
}
...
mod tests {
    ...
    const PING_PACKAGE_PATHS: [&str; 3] = [
        "ping/boxology.toml",
        "ping/implementation/Cargo.toml",
        "ping/implementation/src/lib.rs",
    ];
...
"#;
    ...
    fn embedded_artifacts_match_fixture_corpus() {
        ...
        for (relative, expected) in FIXTURE_CORPUS {
            ...
            if matches!(
                relative,
                "boxology.toml" | "implementation/Cargo.toml" | "implementation/src/lib.rs"
            ) {
                assert_eq!(
                    actual, expected,
