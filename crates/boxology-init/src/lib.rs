...
fn embed_ping() -> Result<Vec<GeneratedFile>, Diagnostics> {
    ...
    let request = GenerationRequest::new(
        ...
        vec![
            ("boxology.toml".into(), PING_MANIFEST.to_vec()),
            (
                "implementation/src/lib.rs".into(),
    ...
    for file in generated.files() {
        let logical = confined_logical(file.path()).map_err(|_| path_not_confined())?;
...
mod tests {
    ...
    const IMPURE_NEEDLES: [&str; 4] = ["std::fs", "std::env", "std::process", "SystemTime"];
    #[rustfmt::skip]
    ...
    fn production_is_pure(production: &str) -> Result<(), String> {
        for needle in IMPURE_NEEDLES {
            if production.contains(needle) {
                return Err(format!("production source must not contain {needle}"));
            }
        }
        Ok(())
    }
    ...
    fn production_inventory_and_code_catalog_fail_closed() {
        ...
        assert_eq!(production.matches("confined_logical(file.path())").count(), 1);
        production_is_pure(production).expect("production source stays pure");
        for needle in IMPURE_NEEDLES {
            let mut mutated = production.to_owned();
            mutated.push_str(needle);
            assert!(production_is_pure(&mutated).is_err(), "{needle}");
        }
        let mut mutated = production.to_owned();
    ...
    fn embedded_generated_paths_are_confined() {
        assert_eq!(confined_logical("generated/contract/src/lib.rs"), Ok("generated/contract/src/lib.rs"));
        assert_eq!(confined_logical("generated/adapter/adapter.rs"), Ok("generated/adapter/adapter.rs"));
        assert_eq!(confined_logical("generated/schema.json"), Ok("generated/schema.json"));
