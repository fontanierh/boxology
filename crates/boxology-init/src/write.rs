...
pub fn write_tree_with(
    ...
    write_tree_at(target, files, &staging, rename)
}
...
mod tests {
    ...
    fn staging_failure_removes_staging_residue() {
        let temp = Temp::new();
        let staging = temp.0.join(".boxology-init-staging-test");
        fs::create_dir(&staging).unwrap();
        fs::write(staging.join("ping"), b"blocker").unwrap();
        let error = write_tree_at(&temp.0, &SAMPLE, &staging, |from, to| fs::rename(from, to)).unwrap_err();
