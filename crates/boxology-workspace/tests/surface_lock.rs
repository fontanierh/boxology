...
use syn::visit::Visit;
const EXPECTED: &str = "BXW0042 BXW0043 BXW0044 BXW0045 BXW0046 BXW0047 BXW0048 BXW0049 BXW0050 BXW0051 BXW0052 BXW0053 BXW0054 BXW0055 BXW0056 BXW0057 BXW0058 BXW0059 BXW0060 BXW0087 BXW0088 BXW0089 BXW0090 BXW0098 BXW0099 BXW0100 BXW0101 BXW0102 BXW0108";
...
const RULES: &str = "ESCAPE DUPLICATE SELF_CLAIM UNOWNED OVERLAP RIVALS BOTH LOCK DOCUMENT UNMAPPED UNMATCHED CLAIMED ROLE SELECTED_BOX SELECTED_SCHEMA SELECTOR_MATCH BINDING_EXPOSURE SELECTED_PROVIDER DIFF_UNOWNED DIFF_AMBIGUOUS DIFF_ACCOUNTABLE DIFF_FOREIGN DIFF_LOCK_SCOPE";
...
const VERIFIED_IMPORT_REGENERATION: &str = "pub fn submitted_diff_ownership_with_verified_regeneration_and_imports(\n    base: &[Package],\n    submitted: &[Package],\n    base_paths: &[RelativePath],\n    changed: &[RelativePath],\n    verified: &[(BoxId, BoxId)],\n    generator_configuration_upgraded: bool,\n    verified_imports: &[(BoxId, BoxId, BoxId)],\n) -> DiffOwnership";
...
const IMPORTED_OWNER_GUARD: &str = "owners.binary_search(imported).is_ok()";
const PROTECTED: &str =
...
impl<'ast> Visit<'ast> for Lock {
    fn visit_attribute(&mut self, attr: &'ast syn::Attribute) {
        ...
        self.bad |=
            !repr && !rustfmt_skip && !allowed.split(' ').any(|name| attr.path().is_ident(name));
...
fn locked_sources<'a>(sources: impl IntoIterator<Item = &'a Source>) -> bool {
    ...
    !lock.bad
        ...
        && lib.text.match_indices(VERIFIED_IMPORT_REGENERATION).count() == 1
        ...
        && lib.text.match_indices(IMPORTED_OWNER_GUARD).count() == 1
...
fn surface_and_live_evasions_are_locked() {
    ...
    rejects(
        ...
        source,
        CARGO_MANIFEST_CHANGE_NEW,
        &CARGO_MANIFEST_CHANGE_NEW.replacen("pub ", "", 1),
    );
    rejects(
        "cargo manifest change path",
        source,
    ...
    rejects(
        "verified import regeneration seam",
        source,
        VERIFIED_IMPORT_REGENERATION,
        &VERIFIED_IMPORT_REGENERATION.replacen("pub ", "", 1),
    );
    rejects(
    ...
    rejects(
        "imported package ownership",
        source,
        IMPORTED_OWNER_GUARD,
        "true",
    );
