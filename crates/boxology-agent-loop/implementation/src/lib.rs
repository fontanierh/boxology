...
#[rustfmt::skip] fn usage(value: &model::TokenUsage) -> Result<(), TurnFailure> { if value.input_tokens.checked_add(value.output_tokens).is_none_or(|known| value.total_tokens < known) { Err(protocol("usage_invalid", true)) } else { Ok(()) } }
