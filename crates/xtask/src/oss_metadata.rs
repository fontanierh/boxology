...
fn check_readme(readme: &str, errors: &mut Vec<String>) {
    for truth in ["early-stage framework", "V0 was completed on 2026-08-09",
        ...
        "Boxology `0.2.0` tools are published on crates.io",
...
fn check(root: &Path) -> Result<(), Vec<String>> {
    ...
    let expected = [("version", "0.2.0"), ("edition", "2024"), ("rust-version", "1.97.1"),
    ...
    match fs::read_to_string(root.join("CHANGELOG.md")) {
        ...
        Ok(text) if text.contains("## [0.2.0] - 2026-08-28") && text.contains("Published the ordered `0.2.0` crate closure")
...
mod tests {
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            for file in ["Cargo.toml", "README.md", "CHANGELOG.md", "LICENSE-MIT", "LICENSE-APACHE"]
    ...
    fn live_metadata_passes_and_required_mutations_fail_closed() {
        ...
        for (file, from, to) in [
            ...
            ("CHANGELOG.md", "Published the ordered `0.2.0` crate closure", "Prepared the ordered `0.2.0` crate closure"),
            ("CHANGELOG.md", "Consumers do not add all 19 published crates", "Consumers add all 19 published crates"),
            ("SECURITY.md", "version `0.2.0`", "version `0.1.1`"),
        ] {
