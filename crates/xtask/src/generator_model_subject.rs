...
const PAYLOAD_DEFERRED_AUTHORITY: &str = "For the current generator, payload-bearing error variants require coordinated schema and contract-emitter support. They are parsed/modelled now, but generation must fail closed until that support lands (#103/#104).";
const PAYLOAD_EMISSION_OPPOSITE: &str = "For the current generator, payload-bearing error variants require coordinated schema and contract-emitter support. They are parsed/modelled now, and generation may proceed before that support lands (#103/#104).";
...
"#,
    ...
    if payload_forward.2
        != "BXG0048 payload/src/lib.rs:1:11-1:19 offending=\"payload-bearing error variants are not yet emittable\" rule=\"payload-bearing error variants require coordinated schema and contract-emitter support\" source=\"specs/s2-contract-generator.md D3\""
    {
