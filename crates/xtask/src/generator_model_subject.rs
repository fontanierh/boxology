...
const PAYLOAD_DEFERRED_AUTHORITY: &str = "For the current generator, one-value error payloads over emittable leaves are emittable; a `Blob` value payload remains fail-closed under BXG0040. Named-field payloads (including empty-named) still require contract-emitter support and remain fail-closed under BXG0048 until that support lands (#104).";
const PAYLOAD_EMISSION_OPPOSITE: &str = "For the current generator, one-value error payloads over emittable leaves are emittable; a `Blob` value payload remains fail-closed under BXG0040. Named-field payloads (including empty-named) still require contract-emitter support and may proceed under BXG0048 before that support lands (#104).";
...
fn payload_authority_locked(spec: &str) -> bool {
    spec.matches(PAYLOAD_DEFERRED_AUTHORITY).count() == 1
        && spec.matches(PAYLOAD_EMISSION_OPPOSITE).count() == 0
}
...
pub(crate) fn run(out: &Path) -> Result<(), String> {
    if !payload_authority_locked(include_str!("../../../specs/s2-contract-generator.md")) {
        return Err("payload deferred-emission authority changed".into());
    }
...
mod tests {
    ...
    fn payload_deferred_authority_mutants_are_rejected() {
        assert!(payload_authority_locked(PAYLOAD_DEFERRED_AUTHORITY));
        for mutant in [
            "",
            &format!("{PAYLOAD_DEFERRED_AUTHORITY}\n{PAYLOAD_DEFERRED_AUTHORITY}"),
            PAYLOAD_EMISSION_OPPOSITE,
            &format!("{PAYLOAD_DEFERRED_AUTHORITY}\n{PAYLOAD_EMISSION_OPPOSITE}"),
        ] {
            assert!(!payload_authority_locked(mutant));
        }
    }
