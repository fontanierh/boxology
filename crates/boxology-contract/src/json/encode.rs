...
/// Stable categories for canonical JSON encoding failures.
pub enum EncodeErrorKind {
    /// A required slot or enum payload is missing.
    MissingValue,
    /// Null is not admitted by the descriptor at this position.
    NullConformance,
    /// The contract value does not match the descriptor-selected representation.
    RepresentationMismatch,
    /// An integer does not fit the descriptor's declared width.
    IntegerRange,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// A payload-free canonical JSON encoding failure.
...
impl fmt::Display for EncodeError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self.0 {
            EncodeErrorKind::MissingValue => "missing result value",
            EncodeErrorKind::NullConformance => "null violates descriptor",
            EncodeErrorKind::RepresentationMismatch => "representation mismatch",
            EncodeErrorKind::IntegerRange => "integer outside descriptor range",
            EncodeErrorKind::UnsupportedDescriptor => "unsupported descriptor",
        })
    }
}
...
#[cfg(test)]
fn encode_result(slot: &SlotValue, descriptor: &TypeDescriptor) -> Result<Vec<u8>, EncodeError> {
    if !is_supported(descriptor) {
        return failure(EncodeErrorKind::UnsupportedDescriptor);
    }
...
/// Encodes one contract slot as canonical, descriptor-guided JSON bytes.
///
/// The descriptor fixes otherwise ambiguous JSON representations such as
/// integer widths, blobs, enum envelopes, presence, and sensitive values.
pub fn encode(slot: &SlotValue, descriptor: &TypeDescriptor) -> Result<Vec<u8>, EncodeError> {
    if !is_supported(descriptor) {
        return failure(EncodeErrorKind::UnsupportedDescriptor);
    }
...
fn encode_slot(
    ...
    match slot {
        SlotValue::Missing => return failure(EncodeErrorKind::MissingValue),
        SlotValue::Null
            if matches!(
                descriptor.view(),
        ...
        SlotValue::Null => return failure(EncodeErrorKind::NullConformance),
...
fn encode_value(
    ...
    if let DescriptorRef::Secret(inner) = descriptor.view() {
        let ValueRef::Sensitive(value) = value else {
            return mismatch();
        };
        return encode_value(output, value.view(), inner);
    }
    if matches!(value, ValueRef::Null) {
        return if matches!(
            ...
            failure(EncodeErrorKind::NullConformance)
        };
    }
    let descriptor = match descriptor.view() {
        ...
        _ => return failure(EncodeErrorKind::UnsupportedDescriptor),
    };
    ...
    if let Some(variant) = variants.iter().find(|variant| variant.tag() == tag) {
        match (variant.payload(), payload) {
            ...
            (VariantPayload::Unit, SlotValue::Missing) => {
                return failure(EncodeErrorKind::MissingValue);
            }
            ...
            (VariantPayload::Value(_), SlotValue::Null) => {
                return failure(EncodeErrorKind::NullConformance);
            }
...
fn signed(
    ...
    if !(min..=max).contains(&value) {
        return failure(EncodeErrorKind::IntegerRange);
    }
...
fn mismatch<T>() -> Result<T, EncodeError> {
    failure(EncodeErrorKind::RepresentationMismatch)
}
fn failure<T>(category: EncodeErrorKind) -> Result<T, EncodeError> {
    Err(EncodeError(category))
}

#[cfg(test)]
mod tests {
    use super::EncodeErrorKind as C;
    use super::*;
    use crate::{
    ...
    fn canonical(slot: SlotValue, descriptor: &D, token: &str) {
        ...
        let tree = crate::json::syntax::parse(
            &first[PREFIX.len()..first.len() - 2],
            crate::json::syntax::Limits::new(first.len(), crate::json::syntax::DEFAULT_DEPTH_LIMIT),
        )
        .unwrap();
        let decoded =
            crate::json::semantic::decode_tree(tree, descriptor, DecodeRole::ConsumerOutput)
                .unwrap();
        assert_eq!(encoded(decoded, descriptor), first);
    }

    fn canonical_request(slot: SlotValue, descriptor: &D, token: &str) {
        let bare = encode(&slot, descriptor).unwrap();
        ...
        let tree = crate::json::syntax::parse(
            &bare,
            crate::json::syntax::Limits::new(bare.len(), crate::json::syntax::DEFAULT_DEPTH_LIMIT),
        )
        .unwrap();
        let decoded =
            crate::json::semantic::decode_tree(tree, descriptor, DecodeRole::ProviderInput)
                .unwrap();
        assert_eq!(encode(&decoded, descriptor).unwrap(), bare);
    }
    ...
    fn request_failures_preserve_closed_categories_and_redaction() {
        ...
        for (slot, descriptor, expected) in cases {
            let error = encode(&slot, &descriptor).unwrap_err();
