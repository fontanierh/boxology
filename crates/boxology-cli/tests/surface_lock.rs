...
const PACKAGE_HASH: u64 = 16_074_969_220_508_011_153;
const CORE_PACKAGE_HASH: u64 = 7_051_827_266_695_580_128;
...
const WALK_HASH: u64 = 5_894_807_802_606_001_890;
...
fn source_surface_is_exact_and_mutation_resistant() {
    ...
    for (anchor, replacement) in [
        (
            "Selection::Generate(Some(package)) => selected_workspace_root(root, package)",
            "Selection::Generate(Some(_)) => None",
        ),
        ("roots.len() == 1", "!roots.is_empty()"),
        (
            "fs::symlink_metadata(path).is_ok_and(|metadata| metadata.is_file())",
