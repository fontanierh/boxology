...
The workspace checker walks from the managed Cargo workspace root, excluding VCS metadata and Cargo build-output directories, and finds every `boxology.toml`. Inside a Git worktree it walks cached paths plus non-ignored untracked paths: ignored dependency and build trees stay outside discovery, while a tracked file remains visible even if a later ignore rule matches it. A nested managed root follows the nearest containing repository's ignore rules. Outside Git, the checker retains its full filesystem walk. Manifest paths are normalized relative to that root. An explicit generation package selected from a partially managed monorepo may resolve one unique descendant managed root; unmanaged siblings remain outside that workspace boundary, and an ambiguous selection resolves none. A platform-kind package may declare owned subtrees as fixture data; a `boxology.toml` inside a declared fixture subtree is that package's owned test material, not a workspace package, and discovery does not descend into it. Absolute paths, `..` escapes, symlink escapes, duplicate package identities, overlapping ownership, and files that classify under no package are rejected.
...
Repository-wide ownership policy, CI, build tooling, generator definitions, and merger enforcement belong to platform packages. Ownership, provenance, and quality-policy declarations are protected control-plane data. For each changed path, the current checker uses base declarations when the exact path exists in the base tree and validated submitted declarations otherwise; modified and deleted files cannot be relabeled, while newly introduced packages and files can be attributed by their submitted manifests. It combines those classifications, requires at least one non-derived owner, accepts additional exact owners, and rejects non-lock derived outputs outside that owner set. A semantic root `boxology-generator.toml` upgrade to the running checker version, classified as non-derived platform source, may carry cross-package regeneration, but only for the exact package/output pairs the same check recreated byte-for-byte; unrelated platform changes and stale or tampered outputs retain BXW0101. An owned generated-schema change may likewise carry an importer's exact regenerated output when both plans are byte-verified; the imported package must own a non-derived change, and stale or tampered trees remain foreign. When the managed root itself has no Git object in the base, the checker attributes the wholly new tree under submitted manifests and permits its initial set to span package owners. It does **not** execute immutable base checker code: the candidate can change the checker and workflow, no merger replay or separation-of-duties service is shipped, and branch protection is not configured by Boxology. Protected declarations therefore flag review-sensitive control-plane changes; they do not prevent self-judging policy edits. Semantic self-protection remains [#17](https://github.com/fontanierh/boxology/issues/17).
...
The shipped checker uses validated submitted declarations for introduced paths, so an atomic new package or newly declared file can pass its reporting checks. That does not define the future merger's stronger new-package creation and approval protocol, which remains tracked in [issue #47](https://github.com/fontanierh/boxology/issues/47); the future protocol above continues to describe base-authorized replay until that choreography is accepted.

The shipped checker also accepts one or more exact non-derived owners in a coordinated diff. It
still rejects unowned or ambiguous paths, rejects derived-only diffs, byte-checks declared outputs,
accepts cross-package output changes during a validated platform generator upgrade only when each
output has matching regeneration evidence, accepts exact downstream importer regeneration only
when the changed imported package owns source in the same diff, validates shared-lockfile scope
across all owners, and runs every package quality command. The
future merger's single-owner reconstruction rule above is intentionally unchanged.
