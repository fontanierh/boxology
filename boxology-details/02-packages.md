...
Ownership proposed by the pull request does not retroactively authorize other paths in that same pull request once the managed root exists in the base. The shipped checker's narrow whole-root bootstrap exception does not define the future merger's stronger new-package creation and approval protocol, which remains tracked in [issue #47](https://github.com/fontanierh/boxology/issues/47).
