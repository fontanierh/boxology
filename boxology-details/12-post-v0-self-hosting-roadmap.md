...
| Product/feature | Current support | Minimum missing | First dogfood evidence | Deferred |
| --- | --- | --- | --- | --- |
| Model completion/agent loop | Governed loop plus complete four-box local harness with correlated `run_turn` and `compact` | Nothing for deterministic H0 | Generated fakes drive exact sequential calls; production binary assembly is exercised | Multiple calls, provider marketplace, multimodal or streaming protocol |
