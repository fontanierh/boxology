...
[Stream definition](../boxology-details/11-v0-streams.md#s7--skill-acceptance-and-stage-2-self-hosting) · Status: **accepted** (one independent review round; findings addressed by the amendment of 2026-07-24)
...
The stream's completion check verifies this spec, and additionally that **every stream S0–S6 has a closed completion check *and* no open `stream:sN` issue** — a closed completion check alone is not stream completeness, since work can be filed after it closes (S0's #272 is exactly this case; a post-closure addition reopens the stream's completion state for this gate's purposes). It also verifies the acceptance record exists. S7-COMPLETE closing **is** the v0 milestone declaration; a dated record announces it and cites the evidence chain. If any prior stream's completion is reopened, S7-COMPLETE reopens with it. The ladder's stage-2 text also names pinned-prior-release generator self-validation; that is impossible before a first pinned release exists (the recorded v0 exclusions publish nothing) and is **deferred to the first-release boundary under #74's first rung — recorded here so S7-COMPLETE's stage-2 claim is scoped, not overstated**.
...
5. After absorption, no platform check exists in both xtask and `boxology check`, and none exists in neither: the full retained set of D5 (links, budget, records, deny, doc tests, whitespace, analyzer probe, determinism subjects and meta-tests) runs under its own names, and the deleted xtask registries are demonstrably manifest-derived.
...
7. S7-COMPLETE's closing comment maps every criterion to evidence, confirms every stream S0–S6 has a closed completion check and no open `stream:sN` issue, scopes the stage-2 claim per D7, and the v0 record exists.
...
T2 lands first for stage-2 adoption's sake (D6); T1 may proceed immediately and in parallel; T5 follows T4; T3 follows T1 and the finished S6 installer — it does not depend on T4/T5, since the acceptance run happens in a greenfield repository. S7-COMPLETE closes the stream and v0 per D7. Dependencies: T3 consumes S6 in full (#332–#336) and the runbook consumes S6's initial-contract shape (#333); T4/T5 consume S5's `check` (#327) and the fixture-opacity mechanism; the whole stream depends on S0–S6.
