...
**Amendment of 2026-08-04 (#522 reconciliation).** D4/AC4's "both platforms" PR-validation claim for `boxology check` on this repository is superseded: V0 product PR validation runs `boxology check --base <event base SHA>` on native macOS ARM64 only. Cross-platform re-proof is owned by [#525](https://github.com/fontanierh/boxology/issues/525).

**Amendment of 2026-08-04 (#531/#327 reconciliation).** This amendment supersedes the stale per-PR product-check claims in D4's "runs green ... as part of its PR validation", D5's bounded-deferral sentence requiring product check in repository PR validation, AC4, T4's "check green in PR validation", and the prior #522 amendment's macOS-only per-PR restatement. Stage-2 self-hosting proof is: (1) adopted root `boxology.toml` manifests govern this repository continuously; (2) one complete green exact-main `boxology check` runs in the dispatched deep workflow cited by S7-COMPLETE. V0 has no per-PR product-check enforcement and claims none; continuous nonduplicative enforcement arrives at #342, whose subsumption rule stays unchanged.
