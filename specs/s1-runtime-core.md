...
- **Object-field wire rule:** an absent `Option<T>` field is omitted, never encoded as `null`; explicit `null` is reserved for `Field<T>::Null`. At the top-level and inside lists or maps, where omission is unavailable, `Option<T>::None` is encoded as `null`.
