...
every non-lock derived output must belong to one of those non-derived owners. A semantic root
`boxology-generator.toml` upgrade to the running checker version, owned as non-derived platform
source, additionally authorizes only the exact `(package, derived output)` trees which the same
check regenerated byte-for-byte. A changed imported schema also authorizes an importer's exact
derived output when both generation plans are byte-verified and the imported package is a
non-derived owner of the diff; stale, tampered, or unrelated importer outputs remain foreign. The
sole bootstrap exception is a managed workspace root with no
Git object at all in the base: its wholly introduced
