...
**Amendment of 2026-08-04 (#531/#327 reconciliation).** This amendment supersedes only the preceding #522 amendment's `boxology check --base <event base SHA>` PR-validation clause. Required PR validation runs no product command; an xtask workflow invariant pins that absence. #531 kept product check off the critical path before #327 made check execute full fmt, workspace Clippy/tests, and declared quality commands. No policy-only or skip product mode exists or should be invented for V0. V0 guarantee: PR validation is changed-crate/repository-invariant evidence; repository-content closure requires one dispatch-only `deep-validation.yml` run (`cargo xtask ci --no-budget`, then full `boxology check`) whose head SHA equals exact main after all remaining V0 content merges and immediately before #343. Run 30872049102 at `3f366867` is prior green evidence that predates the #327 stack and cannot satisfy the final gate. This docs PR does not dispatch the final run.

**Amendment of 2026-08-09 (#342 absorption and speed override).** `cargo xtask ci` now owns exactly one full `boxology check`; root fmt/test/Clippy/key-order duplication and the separate deep-workflow product step are gone. `ci-capstone` and `ci-born-valid` are removed after their unique external-integrity, fixture, and born-valid checks move into the retained aggregate. Derived-output and opaque-fixture format selection comes from live manifests; only Cargo lockfiles remain an unconditional budget exclusion. The planned continuous/per-PR product guarantee is explicitly relaxed: a cold explicit-base check measured about 30 minutes in #342, while exact-main deep runs measured 27:51 and 28:43, exceeding the required job's 20-minute timeout and the user's speed ceiling. Required PR CI therefore keeps root fmt and preserve-order in hygiene, xtask invariants, directly changed crates, conditional root build-graph and opaque-fixture checks, and scoped process-reaper coverage, but runs zero product commands. Full regeneration, classification, Cargo-edge, workspace Clippy/test, and declared-quality failures can reach deep/local validation after merge. No policy-only mode, automatic post-merge trigger, platform matrix, or cross-platform claim is added; #525 remains authoritative for cross-platform work.
