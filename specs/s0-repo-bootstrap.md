...
| checks-macos | fail-fast `boxology check`; PR `cargo xtask ci --base <event base SHA>` (fmt-by-selection, workspace tests, fixture fmt/test, key-order, whitespace, links, records, budget, deny, generation purity, determinism local); main-push `cargo xtask ci --no-budget` adds workspace clippy, editor/rust-analyzer, ignored generator deep tests serially, workspace docs, and fixture clippy/docs; produce normal/meta roots with `determinism-manifest --out macos/` and `--out macos-meta/ --meta-cross`, pack each complete root as an uncompressed tar, and upload both; evidence target `aarch64-apple-darwin` | `[self-hosted, macOS, ARM64, boxology-macos-pr]` |
