...
| checks-macos | fail-fast `boxology check`; PR `cargo xtask ci --base <event base SHA>` (fmt-by-selection, workspace clippy/test, fixture fmt/test, key-order, whitespace, links, records, budget, deny, generation purity, determinism local); main-push `cargo xtask ci --no-budget` adds editor/rust-analyzer, ignored generator deep tests serially, workspace docs, and fixture clippy/docs; produce normal/meta roots with `determinism-manifest --out macos/` and `--out macos-meta/ --meta-cross`, pack each complete root as an uncompressed tar, and upload both; evidence target `aarch64-apple-darwin` | `[self-hosted, macOS, ARM64, boxology-macos-pr]` |
...
- **The `ci` interface is explicit and tiered:** `cargo xtask ci --base <sha>` runs the merge-critical suite including budget; `cargo xtask ci --no-budget` is the main-push/deep mode (no meaningful base), adding editor, documentation, fixture clippy/docs, and ignored generator compiler-matrix tests. Local acceptance runs use `--base origin/main` explicitly; local full validation uses `--no-budget` — no command discovers Git or GitHub context implicitly. Both modes include pinned `deny`, so local/CI parity is exact within each named tier: `cargo xtask deny` verifies the pinned cargo-deny version is installed and errors with the install command otherwise.
