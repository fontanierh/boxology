...
**Current narrow structured subset.** The shared parser/model, format-1 schema writer/strict reader, public generator, and contract macro facade accept ordered named-field structs (including empty structs), nonempty unit-only normal enums, and declaration-before-use local references end to end. Type expressions are exactly a scalar or earlier local base, `Option<Base>`, `Vec<Base>`, or `Option<Vec<Base>>`; generated Rust types/codecs, macro re-exports, descriptors, checker/dispatch glue, typed handles, fakes, and adapters all use that same recursive model. Generated boundary models and domain errors implement deterministic `Default` (the first declared enum/error variant, recursively defaulting its payload); they implement `Eq`, `Hash`, `PartialOrd`, and `Ord` exactly when the complete generated shape supports them. Tolerant enums and errors include an opaque `Unknown` payload and therefore remain `PartialEq` under the stable contract ABI, as do floating-point shapes and structs containing either kind. Rust-only conveniences do not alter schema, revision, or canonical contract encoding. Broader containers, payload-bearing normal enums, forward references, and recursion remain residual work under [#102](https://github.com/fontanierh/boxology/issues/102) and [#104](https://github.com/fontanierh/boxology/issues/104).
...
Each generated Cargo package remains box-specific and workspace-unique (for example `hello-contract`); its implementation-side Rust dependency name is selected by `contract_crate` or defaults to `boxology_generated_contract`. A non-default dependency name is repeated on `#[boxology::implementation(contract_crate = <rust_identifier>)]`, letting implementation checking address the generated package directly even when the authored contract module is not compiled in that crate. Generated adapter glue uses the same selected name, so the Cargo package is still depended on exactly once. That generated crate owns the sole compiled public definition of every boundary type, generated `ContractType`/`ContractError` implementations, provider-checking glue, and the semantic-digest-keyed marker. During normal compilation, `boxology::contract!` emits only imports/facades for those types and requires that marker; it never emits an independent boundary definition. Foreign-language clients consume `schema.json` and generated bindings, not the Rust authoring macro.
