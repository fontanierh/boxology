...
Each generated Cargo package remains box-specific and workspace-unique (for example `hello-contract`); its implementation-side Rust dependency name is selected by `contract_crate` or defaults to `boxology_generated_contract`. The contract macro emits the single hidden re-export consumed by implementation checking and generated adapter glue, so the Cargo package is still depended on exactly once. That generated crate owns the sole compiled public definition of every boundary type, generated `ContractType`/`ContractError` implementations, provider-checking glue, and the semantic-digest-keyed marker. During normal compilation, `boxology::contract!` emits only imports/facades for those types and requires that marker; it never emits an independent boundary definition. Foreign-language clients consume `schema.json` and generated bindings, not the Rust authoring macro.
