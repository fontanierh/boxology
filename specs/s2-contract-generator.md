...
T1's architecture gate in AC7 comes before further grammar or emitter work. It must prove the complete Hello model from the controlled block, pure generation, the one-type facade, and normal-rustc implementation checking. Every generation failure uses D10; generated bytes remain platform-independent.
...
**Amendment of 2026-08-09 (accepted evidence authority).** The AC1, AC6, and AC7 wording above records the already accepted S0/#522/#525 V0 authority rather than introducing a new relaxation: final exact-main native macOS ARM64 evidence, including the registered subject's repeat/root/time/locale/timezone proof, runs in dispatch-only deep validation rather than lean PR CI. Platform-independent output remains a product requirement; [#525](https://github.com/fontanierh/boxology/issues/525) must restore Linux/macOS comparison before the first pinned external release.
