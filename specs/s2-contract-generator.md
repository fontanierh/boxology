...
Remaining load-bearing implementation decisions tracked in [#102](https://github.com/fontanierh/boxology/issues/102) are capability metadata validation; the normative D3 path/type/aggregate/wrapper grammar; the general type/member-name productions; self-import policy—which #102 retains after #99 closed without selecting an assembly rule—and transitive presence through `Secret` coordinated with #112. The crate-root request boundary, deterministic module traversal and reachability, ancestor-`cfg` validation, direct marker-placement validation, structural capability declaration discovery, and structural capability call-frame validation are settled and implemented. No capability metadata, D3 or name grammar, self-import, or transitive-presence authoring behavior may be inferred until its owning decision is accepted. Constructor, request-carrier, and descriptor-conformance behavior is not support policy.
