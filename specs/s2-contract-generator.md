...
**Current narrow structured subset.** The shared parser/model, format-1 schema writer/strict reader, public generator, and contract macro facade accept ordered named-field structs (including empty structs), nonempty unit-only normal enums, and declaration-before-use local references end to end. Type expressions are exactly a scalar or earlier local base, `Option<Base>`, `Vec<Base>`, or `Option<Vec<Base>>`; generated Rust types/codecs, macro re-exports, descriptors, checker/dispatch glue, typed handles, fakes, and adapters all use that same recursive model. Generated boundary models and domain errors implement deterministic `Default` (the first declared enum/error variant, recursively defaulting its payload); they implement `Eq`, `Hash`, `PartialOrd`, and `Ord` exactly when every leaf supports them, so floating-point shapes remain `PartialEq`. Rust-only conveniences do not alter schema, revision, or canonical contract encoding. Broader containers, payload-bearing normal enums, forward references, and recursion remain residual work under [#102](https://github.com/fontanierh/boxology/issues/102) and [#104](https://github.com/fontanierh/boxology/issues/104).
