...
**Current narrow structured subset.** The shared parser/model and format-1 schema writer/strict reader now accept ordered named-field structs (including empty structs), nonempty unit-only normal enums, and declaration-before-use local references. Type expressions are exactly a scalar or earlier local base, `Option<Base>`, `Vec<Base>`, or `Option<Vec<Base>>`. The deterministic emitter now produces the corresponding public Rust types and strict `ContractType` codecs, and the contract macro facade re-exports those types in the focused E1 proof. Complete public generation still fails closed under `BXG0038` before emitting structured descriptors, dispatch, handles, fakes, or adapters. Broader containers, payload-bearing normal enums, forward references, recursion, and complete structured call surfaces remain residual work under [#102](https://github.com/fontanierh/boxology/issues/102), [#104](https://github.com/fontanierh/boxology/issues/104), and the production-driven subset in [#574](https://github.com/fontanierh/boxology/issues/574).
...
The required capability-to-error-enum link resolves over the complete block. Local data references resolve only to an earlier declaration, so self, forward, and recursive references fail closed. Forward-reference resolution and recursive types remain a broader post-v0 residual ([#102](https://github.com/fontanierh/boxology/issues/102)). The narrow type/codec support above is not a claim that complete structured call surfaces or the whole target grammar are implemented.
...
- Descriptors and complete generated call surfaces for [#574](https://github.com/fontanierh/boxology/issues/574)'s delivered narrow type/codec subset remain tracked there; broader structured Rust emission and authoring, forward references, and recursion remain under [#102](https://github.com/fontanierh/boxology/issues/102).
