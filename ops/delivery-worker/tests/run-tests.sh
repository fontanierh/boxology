...
PID=$(/usr/bin/awk -F= '/^pid=/{print $2}' "$DW_STATE_DIR/startwindow.record"); STAGE=$(/usr/bin/awk -F= '/^stage=/{print $2}' "$DW_STATE_DIR/startwindow.record"); track_group "$PID"; /bin/kill -KILL "$SUP"; wait "$SUP" 2>/dev/null; PPID_NOW=; for i in $(jot 500); do PPID_NOW=$(/bin/ps -p "$PID" -o ppid= | /usr/bin/awk '{print $1}'); [[ "$PPID_NOW" == 1 ]] && break; /bin/sleep .01; done; STATUS_RC=75; STATUS_OUT=; for i in $(jot 500); do STATUS_OUT=$(/bin/bash "$S" status --run-id startwindow 2>&1); STATUS_RC=$?; [[ $STATUS_RC -eq 0 ]] && break; /bin/sleep .01; done; OUT=$(DW_KILL="$B/start-kill" /bin/bash "$S" reap --run-id startwindow --dry-run); DRY_RC=$?; DRY_SIGNALS=0; [[ -e "$DW_KILL_LOG" ]] && DRY_SIGNALS=$(wc -l <"$DW_KILL_LOG"); OUT2=$(DW_KILL="$B/start-kill" /bin/bash "$S" reap --run-id startwindow); RC=$?; KILLS=$(/usr/bin/grep -c -- '^-KILL ' "$DW_KILL_LOG")
ok '[[ $STAGE == gated && $PPID_NOW == 1 && $STATUS_RC -eq 0 && "$STATUS_OUT" == *"action=status reason=gated"* && $DRY_RC -eq 0 && $DRY_SIGNALS -eq 0 && "$OUT" == *"action=would-term reason=verified"* && $RC -eq 0 && $KILLS -eq 1 && "$OUT2" == *"action=clear reason=kill"* ]]' "post-release command start remains tracked (stage=$STAGE ppid=$PPID_NOW status_rc=$STATUS_RC status_out=$STATUS_OUT dry_rc=$DRY_RC dry_signals=$DRY_SIGNALS dry_out=$OUT rc=$RC kills=$KILLS out=$OUT2)"
...
cp "$DW_STATE_DIR/lock.record" "$B/lock.saved"; SAVE=$(/sbin/md5 -q "$B/lock.saved"); /usr/bin/awk -F= '/^identity_id=/{split($2,a,":"); print "identity_id=0:" a[2]; next}{print}' "$B/lock.saved" >"$DW_STATE_DIR/lock.record"
OUT=$(/bin/bash "$S" reap --run-id lock 2>&1); RC=$?; /bin/kill -0 "$PID" 2>/dev/null; LIVE=$?
ok '[[ $RC -eq 70 && $LIVE -eq 0 && "$OUT" == *"reason=ambiguous"* ]]' 'changed private identity-handle device fails closed'
