...
set -euo pipefail
umask 077
if [[ -n "$(find /runner -mindepth 1 -maxdepth 1 -print -quit)" ]]; then
  printf '%s\n' 'runner: state volume is not fresh' >&2
  exit 75
fi
cp -a /opt/actions-runner/. /runner/
cd /runner
mkdir -p home _work/.cargo _work/_temp
export HOME=/runner/home CARGO_HOME=/runner/_work/.cargo RUNNER_TEMP=/runner/_work/_temp
